...
// Diferente do app de finanças (um documento gigante fazendo o papel de
// localStorage), o OpenMind usa coleções separadas por tipo de dado
// (answers, studySessions, reviews, vocabularyProgress — cada uma com
// suas próprias funções, criadas junto com a etapa que as usa). Este
// par cuida só do documento de PERFIL: xp, nível, streak, idioma, modo
// de estudo etc.
...
export async function carregarPerfil(uid) {
    ...
    return instantaneo.exists() ? { ...PERFIL_PADRAO, ...instantaneo.data() } : { ...PERFIL_PADRAO };
}

export function salvarPerfil(uid, dadosParciais) {
    const referencia = doc(db, "users", uid);
    return setDoc(referencia, dadosParciais, { merge: true })
        .catch(erro => console.error("Erro ao salvar perfil no Firebase:", erro));
}
